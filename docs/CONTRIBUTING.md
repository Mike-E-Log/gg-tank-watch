...
All tests must pass (currently 213/213 via `python eval/run_all.py --skip integration`). The harness checks safety-critical properties: no fabricated sources, no authored directives, no stale data presented as fresh, provenance on every claim.
