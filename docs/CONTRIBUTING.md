...
All tests must pass (`python eval/run_all.py --skip integration` must exit 0, every test green). The harness checks safety-critical properties: no fabricated sources, no authored directives, no stale data presented as fresh, provenance on every claim.
