...
Structural controls are only worth trusting if you can show they hold, and keep holding as the code changes. So the controls come with a 212-test eval harness (212/212 via `python eval/run_all.py --skip integration`). Its control-specific tests gate corroboration, provenance, freshness, and date sanity. They run every cycle and gate on exit code. A few of them:
