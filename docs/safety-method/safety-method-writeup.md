...
**TL;DR.** During a chemical-tank evacuation that moved about 50,000 people near me, I built a dashboard that used an AI language model (LLM) to summarize official and news sources every 30 minutes. The one failure that could get someone killed is the model confidently saying "it's safe" when it isn't. You can't fix that by prompting carefully. So I bounded the model's authority in the system itself. The tool is a pure information conduit: it routes people to officials and writes no safety verdicts of its own. Every model output passes through one validation checkpoint with asymmetric corroboration gates: stricter checks on all-clear signals than on danger signals. An automated eval harness, with control-specific tests gating corroboration, provenance, freshness, and date sanity, checks the behavior on every update. The design principle is the testable one. You can't write an automated test for "the verdict is correct," but you can write one for "the system never exceeded its authority." This is a small, real instance of scalable oversight (keeping the system's behavior checkable as it operates) and AI control on a deployed consumer system, and the safety work made the product better, not worse.

## The setup

In May 2026 a methyl-methacrylate storage tank at an aerospace facility in Garden Grove, California started venting. About 50,000 people were placed under evacuation. I live downwind. The official channel, the city's emergency page, updated in slow bulk statements. A resident refreshing at 2 a.m. saw the same notice from 6 p.m. The thing residents actually wanted was a current, sourced, plain picture of what changed and what officials were saying. It was scattered across news live-blogs buried in ads, with nothing in Vietnamese for a heavily Vietnamese-speaking area.
...
There's also a failure contract. If the gather step fails (no API key, network down, or the model refuses), it reports failure with empty output. The writer is skipped, `status.json` is left untouched, and the staleness banner fires on schedule. Failure degrades to visibly stale, never to confidently wrong.

This is the part I think of as AI control: keeping the system's actions inside a known, bounded space rather than relying on the model to behave. The system's authority is bounded by design, not by instruction. The corroboration gate doesn't ask the model to be careful. It makes a single hallucinated value unable to reach users as an official-looking all-clear.

## Scalable oversight: the eval harness

Structural controls are only worth trusting if you can show they hold, and keep holding as the code changes. So the controls come with an automated eval harness (every test green via `python eval/run_all.py --skip integration`). Its control-specific tests gate corroboration, provenance, freshness, and date sanity. They run every cycle and gate on exit code. A few of them:
...
- The negligent-undertaking doctrine (Restatement 323 and 324A, a legal reference work that codifies common-law duties) turns on reliance. A conduit that writes no verdict gives reliance nothing to grab onto.
- Negligent misrepresentation (Restatement 552) needs a pecuniary interest (a financial stake in the transaction). A free, non-commercial tool has none.
- Section 230 of U.S. communications law (47 U.S.C. § 230) protects a conduit for other people's content, not for its own words. That's one more reason the summaries stay factual and sourced and the system writes no analysis.
...
- Silent scheduler failure. There's no external heartbeat, so the staleness banner is the only in-app signal, and a banner bug plus a stopped refresh job would have to fail together.
