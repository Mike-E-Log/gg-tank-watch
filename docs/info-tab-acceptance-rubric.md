...
- [ ] **Primary content = 13px across all four panels** (Summary `.info-kv`, Officials `.info-row`, Resources `.shelter-name`/`.info-school-card`, About `.info-ai-disclosure` + `.source-item`) — harmonized 2026-06-03 per user (About's disclosure + source links were 12px outliers). **Role tokens shared by all panels:** descriptor band `.info-desc` 12.5px; **group/section titles 11px/700/caps/`--sa-text-2`** (Shelters/Schools/Recovery `.info-section-title` AND the About "Sources" toggle `.info-sources-toggle`, matched 2026-06-03 so a heading stands out above its 13px items); fine print 11px (`.info-fine`, `.info-row-desc`, sources caption); body prose `.info-who-body` 12px. The AI disclosure renders at body `--sa-text` (not a gold accent — changed 2026-06-02). The Accessibility link sits at the bottom of About (after the Sources fold), not under the disclosure, and is now a **prominent, centered, bordered tappable link** (`.info-a11y-btn`, ≥44px tap target, celadon outline) — no longer the quiet 11px `.info-fine` footer link (made more apparent + accessible 2026-06-03 per user).
