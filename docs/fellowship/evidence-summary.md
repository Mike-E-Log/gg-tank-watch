...
| **Human oversight** | Human reviews all AI summaries before publication. Native-speaker verification required for all non-English safety copy (G1 rule). No machine-translated safety content ships. | `docs/CODE_OF_CONDUCT.md` (principle 6), `docs/LANGUAGE_ACCESS.md` (G1 binding constraint) | Pipeline design: `gather_facts.py` → human review → `update_status.py` → publish |
| **Scalable oversight** | 45-test eval harness monitors behavioral properties across every update cycle. 15 control-specific tests enforce corroboration (P0-1), provenance (P0-2), freshness (P0-3), and date sanity (P1-1). | `docs/AI_CONTROL_ARCHITECTURE.md`, `eval/run_all.py`, `docs/FAILURE_ANALYSIS.md` | `test_lifted_requires_corroboration`, `test_fabricated_source_url_not_in_snapshot`, `test_empty_facts_do_not_advance_data_as_of`, `test_future_resolved_iso_suppressed` |
