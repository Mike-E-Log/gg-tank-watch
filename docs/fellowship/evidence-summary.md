...
| **Scalable oversight** | 48-test eval harness monitors behavioral properties across every update cycle. 17 control-specific tests enforce corroboration (P0-1), provenance (P0-2), freshness (P0-3), date sanity (P1-1), and the G1 language-access gate. | `docs/AI_CONTROL_ARCHITECTURE.md`, `eval/run_all.py`, `docs/FAILURE_ANALYSIS.md` | `test_lifted_requires_corroboration`, `test_fabricated_source_url_not_in_snapshot`, `test_empty_facts_do_not_advance_data_as_of`, `test_future_resolved_iso_suppressed` |
