...
def test_safety_strip_disclaimer_has_no_trailing_period():
    ...
    like a stray double-stop). Guards both the rendered i18n en value and the no-JS fallback span."""
    text = DASHBOARD.read_text(encoding="utf-8")
    m = re.search(r'"safety\.strip\.info":\s*\{\s*en:\s*"([^"]*)"', text)
    en = (m.group(1) if m else "")
    en_ok = bool(en) and not en.rstrip().endswith(".")
    fm = re.search(r'data-i18n="safety\.strip\.info">(.*?)</span>', text, re.S)
    fb = (fm.group(1) if fm else "").strip()
    fb_ok = bool(fb) and not fb.endswith(".")
    ok = en_ok and fb_ok
    return {"passed": ok,
