...
| Testing the safety filter | Some tests are drills: they hand the filter a false report — a lone "all clear," an invented link, a future date — the way you test a smoke alarm with fake smoke. The report is fake; the refusal must be real, from the code as just changed | If a false report gets through, the build fails — the change never goes live |
