...
| Testing the safety filter | Some tests are drills: they hand the filter a made-up lie — a lone "all clear," an invented link, a future date — the way you test a smoke alarm with fake smoke. The lie is fake; the refusal must be real, from the code as just changed | If a lie gets through, the build fails — the change never goes live |
